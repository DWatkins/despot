#include "simple_hallway_rock_sample.h"
...
bool SimpleRockSample::Step(State& state, double rand_num, ACT_TYPE action,
  ...
  if(rover_position == EXIT){
    reward = (rock_status == R_GOOD) ? -1000 : 0;
    return true;
  } else{
    return false;
  }
...
double SimpleRockSample::ObsProb(OBS_TYPE obs, const State& state,
	...
	if (action == A_CHECK) {
		...
		if (rover_position == MIDDLE_LEFT) {
			...
			// when the rover between LEFT_LEFT and EXIT, its observation is correct with probability 0.8
...
State* SimpleRockSample::CreateStartState(string type) const {
  ...
  return new SimpleState(8, R_BAD);
}
...
void SimpleRockSample::PrintBelief(const Belief& belief, ostream& out) const {
	...
	out << "Position belief:"
      << " LEFT_LEFT" << ":" << pos_probs[0]
      << " LEFT_MIDDLE" << ":" << pos_probs[1]
      << " LEFT_RIGHT" << ":" << pos_probs[2]
      << " MIDDLE_LEFT" << ":" << pos_probs[3]
      << " MIDDLE_MIDDLE" << ":" << pos_probs[4]
      << " MIDDLE_RIGHT" << ":" << pos_probs[5]
      << " RIGHT_LEFT" << ":" << pos_probs[6]
      << " RIGHT_MIDDLE" << ":" << pos_probs[7]
      << " RIGHT_RIGHT" << ":" << pos_probs[8]
      << " EXIT" << ":" << pos_probs[9] << endl;
}
